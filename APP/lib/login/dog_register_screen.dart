...
class DogRegisterScreen extends StatefulWidget {
  ...
  const DogRegisterScreen({super.key, this.popOnSuccess = false});
  @override
...
class _DogRegisterScreenState extends State<DogRegisterScreen> {
  ...
  DateTime? birth;
  String get birthText => birth == null ? '선택하세요' : _fmtBirth(birth!);
  ...
  Future<void> _save() async {
    ...
    final body = {
      ...
      'neutered': neutered,
      'weight': wtNum ?? wtText,
      'profileImage': imageCtrl.text.trim().isNotEmpty ? imageCtrl.text.trim() : null,
    };
    ...
    if (rsp.statusCode != null && rsp.statusCode! >= 200 && rsp.statusCode! < 300) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('등록 실패 ($code)'), backgroundColor: Colors.redAccent),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.5,
        centerTitle: true,
        title: const AppFont('반려견 등록', size: 23, fontWeight: FontWeight.w800, color: Colors.black87),
                    ...
                    child: Stack(
                      ...
                      children: [
                        CircleAvatar(
                          ...
                          backgroundImage:
                          imageCtrl.text.isNotEmpty ? NetworkImage(imageCtrl.text) : null,
